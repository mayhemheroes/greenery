#!/usr/bin/env bash
#
# greenery/mayhem/test.sh — RUN greenery own pytest suite and emit a CTRF summary. exit 0 iff no
# test failed.
#
# PATCH-grade behavioral oracle: greenery/*_test.py are known-answer cases asserting regex parsing,
# reduction, FSM equivalence, charclass algebra, multiplier arithmetic, and Pattern set operations
# EXACTLY. A no-op / "exit(0)" / behavior-altering patch to greenery cannot pass it.
#
# The suite is run through the `/mayhem/greenery-tests` ELF launcher (which exec()s
...
# executable. The system `python3` under /usr/bin is SPARED, but `/mayhem/greenery-tests` is not —
...
if [ ! -x /mayhem/greenery-tests ]; then
  echo "FATAL: /mayhem/greenery-tests launcher missing — run mayhem/build.sh first" >&2
  emit_ctrf "pytest" 0 1 0
  exit 2
fi

echo "=== running pytest (greenery known-answer suite) via /mayhem/greenery-tests ==="
out="$(/mayhem/greenery-tests 2>&1)"; rc=$?
echo "$out"
